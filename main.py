...
def ensure_ffmpeg():
    ...
    try:
        ...
        with tf.open(tar_path, "r:xz") as tar:
...
def download_video(url, outdir, cookies=""):
    ...
    found = [os.path.join(outdir, f) for f in os.listdir(outdir) if os.path.splitext(f)[1].lower() in video_exts]
    if not found:
        detail = (r.stderr + "\n" + r.stdout)[-600:]
        raise RuntimeError("Download failed, no video file. " + detail)
    vpath = found[0]
    dur = 0.0
    return vpath, dur
